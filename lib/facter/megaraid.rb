...
class Megaraid
  ...
  def controller_info
    ...
    output = JSON.parse(Facter::Util::Resolution.exec("#{@storcli} /call show J"))
  ...
  def pr_info
    @pr_info = {}
    return unless @storcli
    output = JSON.parse(Facter::Util::Resolution.exec("#{@storcli} /call show patrolread J"))
  ...
  def cc_info
    @cc_info = {}
    return unless @storcli
    output = JSON.parse(Facter::Util::Resolution.exec("#{@storcli} /call show cc J"))
    # this command will return the properties in pairs, transforming into key/value
    output.fetch('Controllers').each do |controller|
      cc_properties = {}
      controller.dig('Response Data', 'Controller Properties').each do |attribute|
